 ...
 *    // L&L added cell type 7 "checkbox"
 *
 *    editable: [
 *
  ...
  var methods = {
    init: function (options) {
      ...
      function ajax(action) {
        ...
        // L&L Modified start - der serializer führt zu falsch codierten Leerzeichen und "+" Symbolen im Callback
        // daher bauen wir den serialize string für den Callback selbst zusammen 
        var i; 
        var s="";
        var tableinput =  $table.find('.tabledit-input'); 
        for (i=0;i<tableinput.length;i++){
          if (tableinput[i].disabled==false){ 
            if (s.length>0)
              s=s+"&";
            s=s+tableinput[i].name+"="+tableinput[i].value;
            //if (tableinput[i].name=="id"){
            //  s1=tableinput[i].name+"="+tableinput[i].value;
          }
        }
        var serialize = tableinput.serialize(); 
       ...
       result = settings.onAjax(action, s);
       // L&L Modified ...end 
       ...
       // In onAjax() it is possible to modify the document sent in ajax request, e.g. adding additional information required by server, or do formatting / restructing of document to be sent.
        ...
        // AJAX
        // L&L modified: wir wollen kein PHP  
        jqXHR=true;
        /*
        jqXHR = $.ajax({
          url: settings.url,
       ...
       * @type {object}
       */      
      // L&L modified: array um eine Dimension erweitert, damit nicht ganze Spalten, sondern einzelne Zellen adressiert werden und Typ checkbox hinzugefügt
      var Draw = {
        columns: {
          identifier: function () {
            ...
            $td.each(function () {
              ...
              if (settings.autoDisable == false) {
                var input = '<input class="tabledit-input tabledit-identifier" type="hidden" name="' + settings.columns.identifier[1] + '" value="' + text + '" >';
              } else {
          ...
          editable: function () {
            for (var j = 0; j < settings.columns.editable.length; j++){
              // falls das array ein leeres Element enthält 
              if (settings.columns.editable[j]==undefined)
                continue; 
              for (var i = 0; i < settings.columns.editable[j].length; i++) {
                //var $td = $table.find('tbody tr:not(".' + settings.noEditClass + '") td:not(".' + settings.noEditClass + '")').filter(':nth-child(' + (parseInt(settings.columns.editable[j][i][0]) + 1) + ')');
                if (settings.columns.editable[j][i]==undefined)
                  continue;
                var $td = $table.find('tbody tr').eq(j).find('td').eq(i);
                ...
                $td.each(function () { 
                  ...
                  var columnType = settings.columns.editable[j][i][2];
                  ...
                  //  var span = '<span class="tabledit-span" style="display: inline;">' + text + '</span>';
                  //else
                  ...
                  switch (columnType) {
                    case 'input':
                      ...
                      if (settings.autoDisable == false) {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" >';
                      }else{
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" disabled>';
                      }
                      break;
                    case 'hidden':
                      // Create text input element.
                      if (settings.autoDisable == false) {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="hidden" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" >';
                      }else {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="hidden" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" disabled>';
                      }
                      break;
                    case 'number':
                      // Create text input element.
                      if (settings.autoDisable == false) {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="number" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" >';
                      }else {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="number" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" disabled>';
                      }
                      break;
                    case 'checkbox':
                      // Create text or checkbox element.
                      var sLabel="";
                      if (AnyToBool(isVarUndefined(JSON.parse(settings.columns.editable[j][i][3])[2]))==false)
                        sLabel=JSON.parse(settings.columns.editable[j][i][3])[2];
                      if (settings.autoDisable == false) {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="checkbox" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" >' + '&nbsp;' + sLabel;
                      }else {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="checkbox" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" disabled>' + '&nbsp;' + sLabel;
                      }
                      break;
                    case 'date':
                      ...
                      // Create text input element.
                      if (settings.autoDisable == false) {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="date" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" >';
                      }else {
                        input = '<input class="tabledit-input ' + settings.inputClass + '" type="date" name="' + settings.columns.editable[j][i][1] + '" value="' + text + '" style="display: none;" disabled>';
                      }
                      break;
                    case 'select':

                      // Check if exists the third parameter of editable array.
                      if (typeof settings.columns.editable[j][i][3] !== 'undefined') {
                        // Create select element.
                        if (settings.autoDisable == false) {
                          input = '<select class="tabledit-input ' + settings.inputClass + '" name="' + settings.columns.editable[j][i][1] + '" style="display: none;" >';
                        }else {
                          input = '<select class="tabledit-input ' + settings.inputClass + '" name="' + settings.columns.editable[j][i][1] + '" style="display: none;" disabled>';
                        }
                        // Create options for select element.
                        $.each($.parseJSON(settings.columns.editable[j][i][3]), function (index, value) {
                          value = $.trim(value);
                    ...
                    case 'textarea':
                      ...
                      input += ' class="tabledit-input ' + settings.inputClass + '" name="' + settings.columns.editable[j][i][1] + '" style="display: none;" disabled>' + text + '</textarea><span class="count_" style="display: none;">' + counttext + '</span>';
...
/*
            // L&L - Test auf checkbox
            if (($('[type=checkbox]',this)).length>0){
              //$(this).find('.tabledit-span').text($('[type=checkbox]',this)[0].checked);
              $(this).find('.tabledit-span').text($('[type=checkbox]',this)[0].checked);
            }else {
    */
          // Disable identifier.
      ...
      var Edit = {
        reset: function (td) {  
          // L&L modified: ursprünglich musste die Eingabe mit <Return> abgeschlossen werden und hier wurde der ursprüngliche Zelleninhalt wiederhergestellt
          //               Damit Eingaben auch ohne abschließendes <Return> übernommen werden, haben wir den ajax-Request hinzugefügt (dieser löst u.a. den 
          //               OnCellEdit-Callback aus) und übernehmen den Input-Text als neuen Zelleninhalt
          // Send AJAX request to server.
          ...
          $(td).each(function () {
            ...
            if ($input.is('select')) {
              ...
              if (($('[type=checkbox]',this)).length>0){
                //$(this).find('.tabledit-span').text($('[type=checkbox]',this)[0].checked);
                $(this).find('.tabledit-span').text($('[type=checkbox]',this)[0].value);
              }else {
                $(this).find('.tabledit-span').text($input.val());
              }
            }
            /*
            // Originalbehandlung: der Input-Text wurde nicht übernommen 
            // Get span text.
      ...
      var Delete = {
        ...
        submit: function (td) {
          ...
          if (settings.autoDisable == false) {
            $(td).closest('tr').find('input.tabledit-identifier').attr('disabled', false);
          }else {
